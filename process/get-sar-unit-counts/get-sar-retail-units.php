...
require_once $processdir.'/nrr-connect.php';
      ...
      "SELECT
      round(sum(`opentapsdb`.`ORDER_ITEM`.`QUANTITY`),0) AS `RetailSARQuantity`
  ...
  WHERE
      LEFT(`opentapsdb`.`ORDER_HEADER`.`ORDER_ID`,4) <> 'TEST'
      AND LEFT(`opentapsdb`.`ORDER_HEADER`.`ORDER_ID`,4) <> 'DEMO'

          AND CAST(`opentapsdb`.`ORDER_HEADER`.`ORDER_DATE` AS DATE) BETWEEN '$startdate' AND '$enddate'

          AND (`opentapsdb`.`ORDER_ITEM`.`PRODUCT_ID` = '1002' or `opentapsdb`.`ORDER_ITEM`.`PRODUCT_ID`='1002AS')
          and isnull(`opentapsdb`.`PARTY_GROUP`.`GROUP_NAME`)
          
          and `opentapsdb`.`ORDER_HEADER`.`ORDER_ID` not in (select `OrderNumber` from `OpentapsDW`.`DeadOrders`)
